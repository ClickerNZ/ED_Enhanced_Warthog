...
$onChanged = Register-ObjectEvent -InputObject $watcher -EventName Changed -MessageData @{ FullPath = $FullPath } -Action {
    param($eventArgs)

    # Retrieve FullPath from MessageData
    $FilePath = $eventArgs.MessageData.FullPath
