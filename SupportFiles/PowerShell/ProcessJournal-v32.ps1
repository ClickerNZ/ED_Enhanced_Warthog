# v32 	- Autopilot feature added - in development 
#		- Fix apostrophy mangling between encoded ASCII and UTF etc
#
# ProcessJournal.ps1
#
...
$Global.statusFile = Join-Path -Path $inputFolderPath -ChildPath "Status.json" 
	...
	#			$Global:newModules = $mask
			...
			"ReceiveText" {
					...
					-not [string]::IsNullOrWhiteSpace($entry.Message_Localised))
				{
					...
					# Enqueue-ATCChatter ("$speaker. " + $entry.Message_Localised) $group
				}
			}
...
#    $statusFile = Join-Path $inputFolderPath 'Status.json'
    if (-not (Test-Path $Global.statusFile)) { 
		$Global:APInvalidReason = "status.json not found"
		return $false
	}

    try {
#        $status = Get-Content -Path $Global.statusFile -Raw -Encoding ascii | ConvertFrom-Json
        $status = Get-Content $Global.statusFile -Raw | ConvertFrom-Json
    } catch {
        return $false
    }
