...
if ($latestFile) {
    ...
    if ($loadGameFound) {
        if ($loadGameData.CMDRName) {
            $loadGameData.CMDRName | Out-File -FilePath "$outputFolderPath\CMDRName.txt" -Encoding UTF8
            Write-Host "CMDRName written to CMDRName.txt"
        }
        if ($loadGameData.ShipType) {
            $loadGameData.ShipType | Out-File -FilePath "$outputFolderPath\ShipType.txt" -Encoding UTF8
            Write-Host "ShipType written to ShipType.txt"
        }
        if ($loadGameData.ShipName) {
            $loadGameData.ShipName | Out-File -FilePath "$outputFolderPath\ShipName.txt" -Encoding UTF8
    ...
    if ($dockedFound) {
        if ($dockedData.StationName) {
            $dockedData.StationName | Out-File -FilePath "$outputFolderPath\StationName.txt" -Encoding UTF8
            Write-Host "StationName written to StationName.txt"
        }
        if ($dockedData.StationType) {
            $dockedData.StationType | Out-File -FilePath "$outputFolderPath\StationType.txt" -Encoding UTF8
    ...
    if ($loadoutFound) {
        if ($loadoutData.ShipName) {
            $loadoutData.ShipName | Out-File -FilePath "$outputFolderPath\ShipName.txt" -Encoding UTF8
            Write-Host "ShipName written to ShipName.txt"
        }
    } else {
        Write-Host "No 'LoadGame' event found in the file."
